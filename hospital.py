...
class hospital:
    def __init__(self, root):
        ...
        comNametablet["values"] = ("Paracetemol", "Corona Vaccine", "Acetaminophen", "Adderall", "Amlodipine", "Ativan")
        ...
        self.txtPrescription.grid(row=0, column=8)
...
#------------------Functionality Declaration-------------------#
    ...
    def update_data(self):
        conn=mysql.connector.connect(host="localhost", username="root", password="Root@123", database="mydata")
        my_cursor=conn.cursor()
        query = "UPDATE hospital SET NameOftablets=%s, dose=%s, Numbersoftablets=%s, lot=%s, issuedate=%s, expdate=%s, dailydose=%s, storage=%s, patientname=%s, nhsnumber=%s, patientaddress=%s WHERE Reference_No=%s"
        values = (
                                                                                                self.Nameoftablets.get(),
                                                                                                self.Dose.get(),
                                                                                                self.NumberofTablets.get(),
